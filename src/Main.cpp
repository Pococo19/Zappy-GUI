...
int main(void)
{
    ...
    skybox.materials[0].shader = LoadShader(TextFormat("skybox.vs", GLSL_VERSION),
                                            TextFormat("skybox.fs", GLSL_VERSION));
    ...
    Shader shdrCubemap = LoadShader(TextFormat("cubemap.vs", GLSL_VERSION),
                                    TextFormat("cubemap.fs", GLSL_VERSION));
    ...
    {
        Image img = LoadImage("skybox2.png");
...
static TextureCubemap GenTextureCubemap(Shader shader, Texture2D panorama, int size, int format)
{
    ...
    unsigned int fbo = rlLoadFramebuffer(size, size); // Corrected function call
